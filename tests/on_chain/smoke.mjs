...
const A = (await cp.waitForTransactionReceipt({ hash: dh, status: "ACCEPTED", retries: 40, interval: 4000 }))?.data?.contract_address;
console.log("Split at", A, "\n");
...
const empty = await send(cp, "open", ["manual-es", payee.address, "Translate the 2,000-word product manual from English to Spanish and deliver it as a .docx by 10 September 2026."], 0n);
ok("an empty escrow is refused", empty.j?.ok === false && String(empty.j?.reason).includes("empty escrow"), empty.j?.reason?.slice(0, 60));
const self = await send(cp, "open", ["manual-es", payer.address, "job"], 3n * GEN);
...
const openA = await send(cp, "open", ["manual-es", payee.address, "Translate the 2,000-word product manual from English to Spanish and deliver it as a .docx by 10 September 2026. Fee: 20 GEN."], 20n * GEN);
ok("a job is funded", openA.j?.ok === true && openA.j?.pool === String(20n * GEN), tally(openA));
const strangerFile = await send(cs, "file", ["manual-es", "I saw the whole thing."]);
...
const early = await send(cs, "judge", ["manual-es"]);
ok("nothing to judge before anybody files", early.exec === "ERROR" && early.msg.includes("nothing to judge"));
const fA1 = await send(cp, "file", ["manual-es", "Nothing was delivered by 10 September. On 12 September I received a .docx containing the English text unchanged, and no reply since."]);
ok("the payer files", fA1.j?.filed_by === "payer" && fA1.j?.both_filed === false);
const again = await send(cp, "file", ["manual-es", "Also it was late."]);
ok("an account is filed once and not rewritten", again.exec === "ERROR" && again.msg.includes("already filed"));
const tooSoon = await send(cs, "judge", ["manual-es"]);
...
const fA2 = await send(ce, "file", ["manual-es", "I had a family emergency and could not do the work. I sent the file back so the client would have it."]);
ok("the payee files, and both sides are in", fA2.j?.both_filed === true);
const noRelease = await send(cp, "release", ["manual-es"]);
ok("a disputed job is not released", noRelease.exec === "ERROR" && noRelease.msg.includes("judged"));
const jA = await send(cs, "judge", ["manual-es"]);
...
const wp = JSON.parse(String(await view("would_pay", ["manual-es"])));
...
const settleEarly = await send(cs, "settle", ["manual-es"]);
...
const twice = await send(cs, "settle", ["manual-es"]);
...
const openB = await send(cp, "open", ["checkout-fix", payee.address, "Fix the checkout page so that orders over 100 GEN no longer fail at payment; deliver a pull request with a regression test by 5 September 2026. Fee: 10 GEN."], 10n * GEN);
ok("case B is funded", openB.j?.ok === true);
await send(cp, "file", ["checkout-fix", "The pull request arrived on 4 September and the fix works. But I also wanted the checkout page redesigned and the payee refused to do it."]);
await send(ce, "file", ["checkout-fix", "The pull request fixes the failing orders and includes a regression test; it was merged on 5 September. A redesign was never part of the job."]);
const jB = await send(cs, "judge", ["checkout-fix"]);
...
const payeeAppeal = await send(ce, "appeal", ["checkout-fix", "I want more."], 1n * GEN);
ok("the winner cannot appeal, and the bond comes back", payeeAppeal.j?.ok === false && String(payeeAppeal.j?.reason).includes("lost"));
const smallBond = await send(cp, "appeal", ["checkout-fix", "The redesign was implied."], (1n * GEN) / 2n);
...
const appealB = await send(cp, "appeal", ["checkout-fix", "The redesign was implied by the word 'page' in the job, so the job is not finished."], 1n * GEN);
...
const second = await send(cp, "appeal", ["checkout-fix", "once more"], 1n * GEN);
ok("there is no second appeal", second.j?.ok === false && String(second.j?.reason).includes("once"));
const sB = await send(cs, "settle", ["checkout-fix"]);
...
await send(cp, "open", ["logo-set", payee.address, "Three logo concepts as SVG by 12 September 2026. Fee: 5 GEN."], 5n * GEN);
const be2 = await balance(payee.address);
const rel = await send(cp, "release", ["logo-set"]);
